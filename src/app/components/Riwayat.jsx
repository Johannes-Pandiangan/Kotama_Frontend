...
const API_URL_GUDANG = "http://localhost:5000/api/riwayat";
const API_URL_SEPATU = "http://localhost:5000/api/riwayat-sepatu";

export function Riwayat() {
  const [historyGudang, setHistoryGudang] = useState([]);
  const [historySepatu, setHistorySepatu] = useState([]);
  ...
  // State Filter Waktu Dinamis (Harian, Bulanan, Tahunan)
  const [timeFilterMode, setTimeFilterMode] = useState("Semua"); // "Semua", "Tanggal", "Bulan", "Tahun"
  const [timeFilterValue, setTimeFilterValue] = useState(""); // Menyimpan isi inputan waktu
  ...
  const fetchRiwayat = async () => {
    try {
      setLoading(true);
      const [resGudang, resSepatu] = await Promise.all([
        fetch(API_URL_GUDANG),
        fetch(API_URL_SEPATU)
      ]);
      const dataGudang = await resGudang.json();
      const dataSepatu = await resSepatu.json();
      
      setHistoryGudang(dataGudang);
      setHistorySepatu(dataSepatu);
    } catch (error) {
  ...
  const filteredHistory = currentData.filter((item) => {
    ...
    // 3. Filter Waktu Dinamis (Harian / Bulanan / Tahunan)
    let matchWaktu = true;
    if (timeFilterMode !== "Semua" && timeFilterValue) {
      ...
      if (timeFilterMode === "Tanggal") {
        // Format input type="date" adalah YYYY-MM-DD
        matchWaktu = `${yyyy}-${mm}-${dd}` === timeFilterValue;
      } else if (timeFilterMode === "Bulan") {
        // Format input type="month" adalah YYYY-MM
        matchWaktu = `${yyyy}-${mm}` === timeFilterValue;
      } else if (timeFilterMode === "Tahun") {
        // Format input type="number" adalah YYYY
        matchWaktu = `${yyyy}` === timeFilterValue;
      }
    }
  ...
  // Reset page jika ada filter yang berubah
  useEffect(() => {
    setPage(1);
  }, [tipeData, filterAktivitas, timeFilterMode, timeFilterValue, search]);
  ...
  const handleExportExcel = () => {
    ...
    if (tipeData === "Bahan Baku") {
      dataToExport = filteredHistory.map((item, index) => ({
        ...
        "Jumlah": item.aktivitas === "masuk" ? `+${item.jumlah}` : `-${item.jumlah}`,
    ...
    <div className="flex-1 flex flex-col min-h-screen" style={{ backgroundColor: "#f5f7fa" }}>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-8 pt-7 pb-5 gap-4">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 text-white px-4 py-2.5 rounded-lg transition-colors shadow-sm outline-none border-none cursor-pointer w-full sm:w-auto hover:bg-emerald-700"
          style={{ backgroundColor: "#107c41", fontSize: "0.875rem", fontWeight: 600 }}
        >
      ...
      <div className="px-4 sm:px-8 pb-8 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-xl px-4 py-3 flex flex-col lg:flex-row items-start lg:items-center gap-4 shadow-sm" style={{ border: "1px solid #e5e7eb" }}>
          ...
          {/* ========================================== */}
          {/* FILTER WAKTU DINAMIS (TANGGAL/BULAN/TAHUN) */}
          {/* ========================================== */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full lg:w-auto">
            <div className="relative w-full sm:w-[130px]">
              <select 
                value={timeFilterMode} 
                onChange={(e) => { setTimeFilterMode(e.target.value); setTimeFilterValue(""); }} 
                className="w-full rounded-lg px-3 py-2 pr-8 outline-none text-sm bg-white border border-gray-200 focus:border-[#0d7a6b] cursor-pointer appearance-none text-gray-700"
              >
            ...
            {timeFilterMode !== "Semua" && (
              <div className="relative w-full sm:w-[160px] flex items-center">
                ...
                {/* Tampilan Input Berubah Tergantung Mode yang Dipilih */}
                {timeFilterMode === "Tanggal" && (
                  <input type="date" value={timeFilterValue} onChange={(e) => setTimeFilterValue(e.target.value)} className="w-full rounded-lg pl-9 pr-4 py-2 outline-none text-sm border focus:border-[#0d7a6b] cursor-pointer" style={{ borderColor: "#e5e7eb", color: timeFilterValue ? "#374151" : "#9ca3af", backgroundColor: "#fff" }} />
                )}
                {timeFilterMode === "Bulan" && (
                  <input type="month" value={timeFilterValue} onChange={(e) => setTimeFilterValue(e.target.value)} className="w-full rounded-lg pl-9 pr-4 py-2 outline-none text-sm border focus:border-[#0d7a6b] cursor-pointer" style={{ borderColor: "#e5e7eb", color: timeFilterValue ? "#374151" : "#9ca3af", backgroundColor: "#fff" }} />
                )}
                {timeFilterMode === "Tahun" && (
                  <input type="number" placeholder="Tahun (cth: 2026)" min="2000" max="2100" value={timeFilterValue} onChange={(e) => setTimeFilterValue(e.target.value)} className="w-full rounded-lg pl-9 pr-4 py-2 outline-none text-sm border focus:border-[#0d7a6b]" style={{ borderColor: "#e5e7eb", color: timeFilterValue ? "#374151" : "#9ca3af", backgroundColor: "#fff" }} />
                )}
              </div>
            )}
          </div>
          {/* Akhir Filter Waktu Dinamis */}
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-gray-50/50 transition-colors" style={{ borderBottom: idx < paginatedHistory.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                          ...
                          <td className="py-4 px-5 text-center">
                            <span className="inline-flex px-3 py-1 rounded-full text-xs font-semibold capitalize" style={{ backgroundColor: isMasuk ? "#e8f5f2" : "#fee2e2", color: isMasuk ? "#0d7a6b" : "#ef4444" }}>
                              {item.aktivitas}
                            </span>
                          </td>
                          <td className="py-4 px-5 text-center font-bold" style={{ fontSize: "0.875rem", color: isMasuk ? "#0d7a6b" : "#ef4444" }}>
                            {isMasuk ? `+${item.jumlah}` : `-${item.jumlah}`}
                          </td>
                          <td className="py-4 px-5 text-center" style={{ fontSize: "0.875rem", color: "#111827", fontWeight: 500 }}>
                            {item.pengambil || "-"}
                          </td>
                          <td className="py-4 px-5 text-center text-gray-500" style={{ fontSize: "0.8125rem" }}>
                            {formattedDate}
                          </td>
                        </tr>
                      );
