...
const API_GUDANG = "http://localhost:5000/api/gudang";
const API_RIWAYAT = "http://localhost:5000/api/riwayat";
const API_BARANG_JADI = "http://localhost:5000/api/barang-jadi";
...
export function Beranda() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        const [resGudang, resRiwayat, resBarangJadi] = await Promise.all([
          fetch(API_GUDANG),
          fetch(API_RIWAYAT),
          fetch(API_BARANG_JADI)
        ]);
        
        const gudang = await resGudang.json();
        const riwayat = await resRiwayat.json();
        const barangJadi = await resBarangJadi.json();
  ...
  // Update List Kategori (Aksesoris dihapus, ditambah Lapis dan Pelengkap)
  ...
  // Grafik berdasarkan Kategori DAN Filter Bulan
  const generateChartData = () => {
    ...
    // Filter riwayat yang sesuai dengan Kategori & Bulan yang dipilih
    ...
    riwayatKategoriBulan.forEach((r) => {
      ...
      if (r.aktivitas === "masuk") {
        weeks[weekIndex].masuk += r.jumlah;
      } else if (r.aktivitas === "keluar") {
        weeks[weekIndex].keluar += r.jumlah;
      }
    });
  ...
  const filteredSearch = lowStockItemsAll.filter((item) =>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-8 pt-7 pb-5 gap-4">
        <div>
          ...
          <p style={{ fontSize: "0.875rem", color: "#6b7280", marginTop: 4 }}>
            Ringkasan data real-time material gudang
          </p>
        </div>
        ...
        <div className="relative w-full sm:w-auto z-20">
          <button
            ...
            className="flex items-center gap-2 bg-white rounded-lg px-4 py-2.5 shadow-sm w-full sm:w-auto"
            style={{ border: "1px solid #e5e7eb", fontSize: "0.9rem", fontWeight: 500, color: "#374151", cursor: "pointer", justifyContent: "space-between" }}
          >
            ...
            <div className="absolute right-0 sm:right-0 left-0 sm:left-auto mt-1 bg-white rounded-lg shadow-lg w-full sm:min-w-[160px]" style={{ border: "1px solid #e5e7eb" }}>
              {KATEGORI_LIST.map((cat) => (
                <button
                  ...
                  style={{
                    fontSize: "0.875rem", color: cat === category ? "#0d7a6b" : "#374151", fontWeight: cat === category ? 600 : 400,
                    backgroundColor: cat === category ? "#e8f5f2" : "transparent", border: "none", cursor: "pointer",
                  }}
                >
                  {cat}
                </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-xl p-5 shadow-sm relative" style={{ border: "1px solid #fecaca", backgroundColor: "#fff5f5" }}>
            <div className="flex items-center justify-between">
              <p style={{ fontSize: "0.7rem", fontWeight: 600, color: "#ef4444", letterSpacing: "0.08em", textTransform: "uppercase" }}>Hampir Habis</p>
            ...
            {!isSepatu && (
              <div className="relative">
                ...
                <input 
                  ...
                  className="rounded-lg pl-9 pr-4 py-2 outline-none border text-sm"
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ border: "1px solid #e5e7eb" }}>
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between px-4 sm:px-6 pt-5 pb-4 gap-4">
            <div className="flex items-center gap-2">
              <AlertTriangle size={18} style={{ color: "#ef4444" }} />
              <h2 style={{ fontSize: "1rem", fontWeight: 600, color: "#111827" }}>Peringatan Stok Rendah</h2>
            </div>
            <div className="relative w-full sm:w-auto">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "#9ca3af" }} />
              <input
                type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder="Cari barang"
                className="w-full sm:w-[200px] rounded-lg pl-8 pr-4 py-2 outline-none"
                style={{ border: "1px solid #e5e7eb", fontSize: "0.8125rem", color: "#374151" }}
              />
            </div>
          </div>
                    ...
                    return (
                      <tr key={item.id} style={{ borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <td className="py-3 px-4 sm:px-6">
                          <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: status.bg, color: status.text }}>
                            {item.status || (item.stok === 0 ? "Habis" : "Menipis")}
                          </span>
                        </td>
                      </tr>
                    );
